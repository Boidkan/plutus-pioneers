...
mkValidator datum _ context = (traceIfFalse "Beneficiary Signature Missing" isSignedByBeneficiary1 &&
                               traceIfFalse "Deadline has not passed" canBeneficiary1Grab) ||
                              (traceIfFalse "Beneficiary Signature Missing" isSignedByBeneficiary2 &&
                              traceIfFalse "Deadline has passed" canBeneficiary2Grab)
    ...
    where
        ...
        isSignedByBeneficiary1 = txSignedBy info $ beneficiary1 datum

        isSignedByBeneficiary2 :: Bool
        isSignedByBeneficiary2 = txSignedBy info $ beneficiary2 datum

        canBeneficiary1Grab :: Bool
        -- Is before/at deadline
        canBeneficiary1Grab = contains (to $ deadline datum) $ (txInfoValidRange info)

        canBeneficiary2Grab :: Bool
        -- Is after deadline
        canBeneficiary2Grab = contains (from $ (1 + deadline datum)) $ (txInfoValidRange info)
