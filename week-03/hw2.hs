...
mkValidator pubKey deadline _ context = traceIfFalse "Beneficiary Signature Missing" isSignedByBeneficiary &&
                                        traceIfFalse "Deadline has not been reached" deadlineReached
    ...
    where
        ...
        isSignedByBeneficiary = txSignedBy info $ pubKey
...
typedValidator pubKey = Scripts.mkTypedValidator @Vesting
    ($$(PlutusTx.compile [|| mkValidator ||]) `PlutusTx.applyCode` PlutusTx.liftCode pubKey)
